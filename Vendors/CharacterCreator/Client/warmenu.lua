...
function WarMenu.TickBar(text1, text2, index,tickOffset, callback)
    ...
    if menus[currentMenu] then
        ...
        if isCurrent then
            if (pressed == keys.right) and (index<100) then


                index=index+1


            elseif	(pressed == keys.left) and (index>-0) then
        ...
        if multiplier then
            ...
            drawRect(x+tickOffset, y, menuWidth-0.42, buttonHeight-0.047, menus[currentMenu].titleColor)
            drawRect(x+tickOffset+offset, y, menuWidth-0.327, buttonHeight-0.047, menus[currentMenu].menuTextColor)
